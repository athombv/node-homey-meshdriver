		...
		this.node.endpoints[capabilityReportObj.endpoint].clusters[clusterId].bind()
			.then(() => {
				this.node.on('command', (command) => {
					...
					// Ignore error if capability is not registered for reports not associated with a capability
					const parsedPayload = this._onReport(capabilityId, command.value);
					if (this._reportListeners[command.cluster]
						&& this._reportListeners[command.cluster][command.attr]) {
						this._reportListeners[command.cluster][command.attr](command.value, parsedPayload);
					}
				});
			})
