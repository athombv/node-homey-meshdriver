...
class ZigBeeDevice extends MeshDevice {
	...
	async registerAttrReportListener(clusterId, attrId, minInt, maxInt, repChange = null, triggerFn, endpointId = 0) {
		...
		const alreadyRegistered = Object.keys(this._attrReportListeners[clusterEndpointId][attrId] || {}).length > 0;
		this._attrReportListeners[clusterEndpointId][attrId] = triggerFn;

		// Make sure to configure just once
		if (alreadyConfigured || alreadyRegistered) {
			this.log(`registerAttrReportListener() -> already configured attr reporting ${endpointId}_${clusterId}_${attrId}`);

			// Listen on this cluster for specific commands
			this.node.on('report', report => {
				if ((this._attrReportListeners[clusterEndpointId]
						&& this._attrReportListeners[clusterEndpointId][report.attr]
						&& attrId === report.attr)) {
	...
	async _configureReport() {
		...
		const clusterEndpointId = `${configureReportObj.endpointId}_${configureReportObj.clusterId}`;
		...
		try {
			...
			else this.configureReportInProcess = false;
			return;
		}
		...
		this.setStoreValue(configureReportObj.reportId, true);

		// Listen for incoming reports
		this.node.on('report', report => {
			if ((this._attrReportListeners[clusterEndpointId]
					&& this._attrReportListeners[clusterEndpointId][report.attr]
					&& configureReportObj.attrId === report.attr)) {
				this._attrReportListeners[clusterEndpointId][report.attr](report.value);
			}
		});
