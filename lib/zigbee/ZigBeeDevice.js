...
class ZigBeeDevice extends MeshDevice {
	...
	_getCapabilityValue(capabilityId, clusterId) {
		...
		try {
			...
			return cluster.read(capabilityGetObj.get)
	 ...
	 * @private
	 */
	_registerCapabilityRealtime(capabilityId, clusterId) {
		...
		this.node.endpoints[capabilityReportObj.endpoint].clusters[clusterId].bind()
			.then(() => {
				this.node.on('command', (command) => {
					// TODO parse command to check for correct command and get payload
					const payload = null;

					const parsedPayload = this._onReport(capabilityId, payload);
					if (parsedPayload instanceof Error) return;

					if (this._reportListeners[clusterId]
						&& this._reportListeners[clusterId][command.name]) {
						this._reportListeners[clusterId][command.name](payload, parsedPayload);
					}
				});
			})
			.catch(err => this.log(`could not bind ${clusterId} cluster`, err));
	}
	...
	registerCapability(capabilityId, clusterId, userOpts) {
		...
		this._registerCapabilityRealtime(capabilityId, clusterId);
	 ...
	 */
	registerReportListener(clusterId, commandId, triggerFn) {
