...
class ZigBeeDevice extends MeshDevice {
	...
	registerReportListener(clusterId, commandId, triggerFn, endpointId = 0) {
		...
		const alreadyRegistered = Object.keys(this._reportListeners[clusterId] || {}).length > 0;
		...
		if (alreadyBound || alreadyRegistered) {
			...
			// Not bound before, try binding until success
			this.node.endpoints[endpointId].clusters[clusterId].bind()
				.then(() => {
					this.setStoreValue(reportId, true);
					this.log(`registerReportListener() -> bind ${clusterId} on endpoint ${endpointId} successful`);
					this.node.on('command', command => {
						if (this._reportListeners[command.cluster]
							&& this._reportListeners[command.cluster][command.attr]
							&& commandId === command.attr) {
