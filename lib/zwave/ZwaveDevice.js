...
class ZwaveDevice extends MeshDevice {
	...
	async onMeshInit(options = {}) {
		...
		// Only register system capabilities if allowed
		if (options.autoRegisterSystemCapabilities === true) {
			await this.registerSystemCapabilities();
		}
	}

	/**
	 * Method that automatically searches the devices capabilities and supported command classes to find all system
	 * capability matches. The matches will then be registered as it is very likely the device is using it for the
	...
	async registerSystemCapabilities() {
		...
		// Add all system capabilities command class matches
		for (let i in capabilities) {
			const capabilityId = capabilities[i];

			// Get all command classes for this capability
			try {
				let files = await fs.readdir(`./system/capabilities/${capabilityId}`);

				try {
					files = files.concat(await fs.readdir(`./system/capabilities/${capabilityId}/${this.getClass()}`));

					// Get unique to prevent registering multiple times
					files = files.filter((value, index, self) => self.indexOf(value) === index);
					this.log(`found device class ${this.getClass()} specific system capability`);
				} catch (err) {
					// Could not find nested dir, continue as normal
				}

				const commandClassFileNames = files.filter(file => file.indexOf('.js') !== -1);
				commandClassFileNames.forEach(commandClassFileName => {
					const commandClassId = commandClassFileName.substr(0, commandClassFileName.lastIndexOf('.'));

					// If device has the command class register it
					if (this.hasCommandClass(commandClassId)) {
						this.log(`registered ${capabilityId} and ${commandClassId}}`);
						this.registerCapability(capabilityId, commandClassId);
					}
				});
			} catch(err) {
				this.error(`failed to read dir ./system/capabilities/${capabilityId}`);
			}
		}
	}
