...
class ZwaveDevice extends MeshDevice {
	...
	*/
	...
	_registerCapabilityRealtime( capabilityId, commandClassId ) {
		...
		if( capabilityReportObj instanceof Error ) return capabilityReportObj;
			...
			commandClass.on('report', ( command, payload ) => {
				...
				if( this._triggers[ commandClassId ]
				 && this._triggers[ commandClassId ][ command.name ] ) {
					 this._triggers[ commandClassId ][ command.name ]( payload, parsedPayload );
				 }
			});
	}
	...
	_onReport( capabilityId, payload ) {
		...
		console.log('capabilityReportObj', capabilityReportObj)
	...
	*/
	...
	registerTrigger( commandClassId, commandId, triggerFn ) {
		this._triggers[ commandClassId ] = this._triggers[ commandClassId ] || {};
		this._triggers[ commandClassId ][ commandId ] = triggerFn;
	}
