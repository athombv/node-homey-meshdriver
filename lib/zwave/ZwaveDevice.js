...
class ZwaveDevice extends MeshDevice {
	 ...
	 * the parameter {@link ZwaveDevice#registerCapability} [opts.getOpts.pollInterval]
	...
	_registerCapabilityGet(capabilityId, commandClassId) {
		...
		if (capabilityGetObj.opts.pollInterval) {
			...
			let pollInterval;
			let pollMultiplication = capabilityGetObj.opts.pollMultiplication || 1;
	...
	_parseCommandClassPayload( commandClassId, payload ) {
		let parser = commandClassParsers[commandClassId];
