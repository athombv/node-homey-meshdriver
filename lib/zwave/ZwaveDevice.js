...
class ZwaveDevice extends MeshDevice {
	 ...
	 * @param {Boolean} [opts.reportParserOverride] - Boolean flag to determine if the `reportParser` method should override all report parsers. (Assumed true when `reportParser` without version is declared)
	...
	registerCapability(capabilityId, commandClassId, userOpts) {
        ...
        if (userOpts) {
            userOpts.reportParserOverride = typeof userOpts.reportParser === 'function' && (!userOpts.reportParserOverride || userOpts.reportParserOverride !== false);
        }
