...
class ZwaveDevice extends MeshDevice {
		...
		this.registerMultipleCapabilityListener(capabilitiesOpts.map(x => x.capability), async (valueObj, optsObj) => {
			...
			if (!result || result instanceof Error) {
				...
				for (const capabilityId of Object.keys(valueObj)) {
					...
					const commandClassId = capabilityObj.commandClass;
					...
					if (capabilitySetObj instanceof Error) {
						this.error(`capabilitySetObj ${capabilityId} ${commandClassId} is error`, capabilitySetObj);
						break;
					}
	 ...
	 * @private
	 */
	_mergeSystemAndUserOpts(capabilityId, commandClassId, userOpts) {
		let systemOpts = {};
		try {
	...
	registerCapability(capabilityId, commandClassId, userOpts) {
		...
		this._mergeSystemAndUserOpts(capabilityId, commandClassId, userOpts);
