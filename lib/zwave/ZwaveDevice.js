...
class ZwaveDevice extends MeshDevice {
	...
	_registerCapabilityGet(capabilityId, commandClassId) {
		...
		if (capabilityGetObj.opts.getOnStart) {
			...
			if (this.node.battery === false) this._getCapabilityValue(capabilityId, commandClassId).catch(err => this.error('Could not get capability value', err));
		...
		if (capabilityGetObj.opts.getOnOnline) {
			...
			if (this.node.battery === true && this.node.online === true) {
				...
				this._getCapabilityValue(capabilityId, commandClassId).catch(err => this.error('Could not get capability value', err));
			}
  ...
  _registerCapabilitySet(capabilityId, commandClassId) {
	...
	registerCapability(capabilityId, commandClassId, userOpts) {
		...
		let systemOpts = {};
		try {
			// Get root capability in case of extended capabilities (e.g. onoff.output1)
