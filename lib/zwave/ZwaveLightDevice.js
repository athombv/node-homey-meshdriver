...
class ZwaveLightDevice extends ZwaveDevice {
	...
	async _sendColors({ warm, cold, red, green, blue, duration }) {
		...
		if (SwitchColorVersion === 3) {
			setCommand = new Buffer([setCommand.Properties1['Color Component Count'], 0, setCommand.vg1[0].Value, 1, setCommand.vg1[1].Value, 2, setCommand.vg1[2].Value, 3, setCommand.vg1[3].Value, 4, setCommand.vg1[4].Value, 255]);
		} else if (SwitchColorVersion === 2) {
